layui.use(['form', 'layer', 'table', 'laytpl'], function () {
    ...
    var tableIns = table.render({
        ...
        url: '/ClassAttendance/biz/class_findByPage.action',//数据接口
        page: true,//开启分页
        ...
        cols: [[//表头
            ...
            {field: 'collegeName', title: '学院名称', minWidth: 100, align: 'center', templet:function(d){
                    return d.tbCollegeEntity.collegeName;
                }},
            {field: 'classGrade', title: '年级名称', minWidth: 100, align: 'center'},
            {field: 'classMajor', title: '专业名称', minWidth: 100, align: 'center'},
    ...
    $(".delBtn").click(function () {
        ...
        if (data.length > 0) {
            ...
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("/ClassAttendance/biz/class_deleteBatch.action", {
    ...
    table.on('tool(infoList)', function (obj) {
        ...
        if (layEvent === 'edit') { //编辑
            ...
            layer.confirm('确定删除此记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get("/ClassAttendance/biz/class_delete.action", {
        ...
        var index = layui.layer.open({
            ...
            success: function (layero, index) {
                ...
                if (edit) {
                    ...
                    body.find(".collegeHide").val(edit.tbCollegeEntity.id);//使用中间变量渲染下拉框
                    body.find(".classGrade").val(edit.classGrade);
                    body.find(".classMajor").val(edit.classMajor);
    ...
    function detailUser(data) {
        var index = layui.layer.open({
            ...
            success: function (layero, index) {
                ...
                body.find(".collegeName").val(data.tbCollegeEntity.collegeName);
                body.find(".classGrade").val(data.classGrade);
                body.find(".classMajor").val(data.classMajor);
                form.render();
            }
        });
        layui.layer.full(index);  //全屏
