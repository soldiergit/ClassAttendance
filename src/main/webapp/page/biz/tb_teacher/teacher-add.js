layui.use(['form', 'layer'], function () {
    ...
    form.on("submit(addUser)", function (data) {
        ...
        // 实际使用时的提交信息
        $.post(updateFlag === '0' ? "/ClassAttendance/biz/teacher_save.action" : "/ClassAttendance/biz/teacher_update.action", {
            id: updateFlag === '0' ? null : $(".Id").val(),//id
            teacherCode: $(".teacherCode").val(),
            teacherName: $(".teacherName").val(),
            teacherSex: data.field.teacherSex,
            collegeId: $("#collegeId").val()
        }, function (res) {
            if (res.code === 0) {
                top.layer.close(index);
                top.layer.msg(updateFlag === '0' ? "添加教师成功！" : "修改教师成功！");
