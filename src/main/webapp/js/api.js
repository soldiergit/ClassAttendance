...
layui.define(['$tool','jquery'], function (exports) {
     ...
     * 封装一个get
     * */
    function doGet(url,req,successCallback,errorCallback,async) {
        $.ajax({
            url:url,
            data:req,
            method:"get",
    ...
    var API = {
        GetAreaList: function(req,successCallback,errorCallback){ // 请求区域
            doGet($tool.getContext() + "sysJson/area_selectAllArea.action",req,successCallback,errorCallback,false);
        },
        UpdateAdminPWd: function(req,successCallback,errorCallback){ // 登录
            doPost($tool.getContext() + "login_updatePassword.action",req,successCallback,errorCallback);
        },
